# game-player-session-actor Specification

## Purpose

规定 **game-service** 如何按 **`playerId`** 维护 **Typed PlayerSessionActor**（在线会话串行边界）、**PlayerSessionRegistry** 的 **流级引用计数** 与 **释放** 语义；**不**规定地图/城权威（阶段 4+）。
...
- **WHEN** 审阅 `openspec/changes/archive/2026-04-14-add-player-session-actor/` 下 `design.md` 或本规格
